...
const PRIVATE_PERSONA_NAME = `${ORG_SCOPE}/persona`;
...
const sourcePackages: readonly SourcePackage[] = [
  ...
  },
  {
    ...
    privateName: PRIVATE_PERSONA_NAME,
...
const privateReadme = (
  ...
  if (source.privateName === PRIVATE_CORE_NAME) {
    return [
      ...
      "",
      "This package is a transitive dependency of `@hourglass-financial/persona`.",
      "",
    ].join("\n");
  }

  return [
    `# ${PRIVATE_PERSONA_NAME}`,
    "",
    "Private GitHub Packages build of the Persona SDK for Hourglass internal repositories.",
    "",
    "## Installation",
    "",
    "```bash",
    `bun add ${PRIVATE_PERSONA_NAME}@persona-sdk effect@${effectPolicy.effectVersion}`,
    "```",
    "",
    "## Usage",
    "",
    "```typescript",
    `import { CredentialsFromEnv } from "${PRIVATE_PERSONA_NAME}";`,
    `import { listAllAccounts } from "${PRIVATE_PERSONA_NAME}/Operations";`,
    "```",
    "",
    "Set `PERSONA_API_KEY` in the consuming environment.",
    "",
  ].join("\n");
};
...
const dependencyOverridesFor = (
  ...
  source.privateName === PRIVATE_PERSONA_NAME
    ? {
...
export const prepareGithubPersonaPackages = async (
...
const usage = (): string =>
  [
    "Usage: bun scripts/prepare-github-persona-packages.ts --version <version> --stage-dir <dir> [--force]",
    "",
    "Creates publish-ready package directories for @hourglass-financial/distilled-core",
    "and @hourglass-financial/persona.",
    "",
...
const parseArgs = (argv: readonly string[]): PrepareOptions => {
  let stageDir = ".ai-workspace/github-persona-packages";
...
if (import.meta.main) {
  try {
    const prepared = await prepareGithubPersonaPackages(
