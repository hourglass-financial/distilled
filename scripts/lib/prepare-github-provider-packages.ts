...
export interface GithubProviderPackageConfig {
  readonly sourceName: string;
  readonly privateName: string;
  readonly directory: string;
  ...
  readonly exampleOperation: string;
  readonly allowCatalogEffectPeer?: boolean;
  readonly scriptName: string;
  readonly defaultStageDir: string;
}

const sourcePackages = (
  config: GithubProviderPackageConfig,
...
// emitted declarations they ship as unresolvable imports.
...
const privateReadme = (
  ...
  if (source.privateName === PRIVATE_CORE_NAME) {
    return [
      ...
      "",
      `This package is a transitive dependency of \`${config.privateName}\`.`,
      "",
    ].join("\n");
  }
  ...
  return [
    `# ${config.privateName}`,
    "",
    `Private GitHub Packages build of the ${config.displayName} SDK for Hourglass internal repositories.`,
    "",
    "## Installation",
    "",
    "```bash",
    `bun add ${config.privateName}@${config.distTag} effect@${effectPolicy.effectVersion}`,
    "```",
    "",
    "## Usage",
    "",
    "```typescript",
    `import { CredentialsFromEnv } from "${config.privateName}";`,
    `import { ${config.exampleOperation} } from "${config.privateName}/Operations";`,
    "```",
    "",
    `Set \`${config.credentialEnvironmentVariable}\` in the consuming environment.`,
    "",
  ].join("\n");
};
...
const dependencyOverridesFor = (
  ...
  source.privateName === config.privateName
    ? {
...
const stagedManifest = (
  ...
  const dependencies = resolveDependencyMap(
    ...
    rootManifest,
    dependencyOverridesFor(source, config),
    version,
  );
...
export const prepareGithubProviderPackages = async (
  ...
  for (const source of sourcePackages(config)) {
    ...
    assertSourceEffectPackagePolicy(
      ...
      {
        allowCatalogPeer:
          source.privateName === PRIVATE_CORE_NAME ||
          (source.privateName === config.privateName &&
            config.allowCatalogEffectPeer === true),
      },
    );
    ...
    await writeFile(
      ...
      privateReadme(source, effectPolicy, config),
    );
...
const usage = (config: GithubProviderPackageConfig): string =>
  [
    `Usage: bun scripts/${config.scriptName} --version <version> --stage-dir <dir> [--force]`,
    "",
    `Creates publish-ready package directories for ${PRIVATE_CORE_NAME}`,
    `and ${config.privateName}.`,
    "",
...
const parseArgs = (
  ...
  let stageDir = config.defaultStageDir;
...
export const runPrepareGithubProviderPackagesCli = async (
  config: GithubProviderPackageConfig,
  argv: readonly string[] = process.argv.slice(2),
): Promise<void> => {
  try {
    const options = parseArgs(config, argv);
    if (options === "help") {
      console.log(usage(config));
      return;
    }
    const prepared = await prepareGithubProviderPackages(options, config);
    for (const pkg of prepared) console.log(`${pkg.name} -> ${pkg.directory}`);
  } catch (error) {
