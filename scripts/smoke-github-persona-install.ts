...
const DEFAULT_PACKAGE = "@hourglass-financial/persona";
const DEFAULT_TAG = "persona-sdk";
...
const buildSmokePlan = (options: SmokeOptions): SmokePlan => {
  ...
  return {
    ...
    importCommand:
      options.packageManager === "bun"
        ? ["bun", "./check-persona-import.mjs"]
        : ["node", "./check-persona-import.mjs"],
  };
};
...
const runSmokeInstall = async (
  ...
  const tempDir = await mkdtemp(path.join(os.tmpdir(), "persona-smoke-"));
  ...
  await writeFile(
    path.join(tempDir, "check-persona-import.mjs"),
    [
      ...
      "if (!mod) throw new Error('Persona package import returned no module');",
      "console.log('Persona package import succeeded');",
      "",
    ].join("\n"),
  );
...
const usage = (): string =>
  [
    "Usage: bun scripts/smoke-github-persona-install.ts [options]",
    "",
    "Options:",
    "  --package <name>          Package to install (default: @hourglass-financial/persona)",
    "  --tag <tag>                Version or dist-tag (default: persona-sdk)",
