...
describe("AuthorizationControllerCheck", () => {
  it("fails with NotFound for a non-existent organization membership id", async () => {
  ...
  it("fails with Forbidden when checking a membership outside this tenant", async () => {
    const error = await runEffect(
      AuthorizationControllerCheck({
  ...
  it("fails with UnprocessableEntity when the membership id is malformed", async () => {
    const error = await runEffect(
      AuthorizationControllerCheck({
