...
describe("RadarStandaloneControllerUpdateRadarAttempt", () => {
  it("updates a Radar attempt's status", async (ctx) => {
  ...
  it("fails with NotFound for a non-existent attempt id", async () => {
  ...
  it("rejects an invalid attempt_status before transport", async () => {
    const error = await runEffect(
      RadarStandaloneControllerUpdateRadarAttempt({
        id: `radar_attempt_${testRunId}`,
        attempt_status: "not-a-valid-status" as "success",
      }).pipe(Effect.flip),
    );
