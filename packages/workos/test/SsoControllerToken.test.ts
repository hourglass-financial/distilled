...
describe("SsoControllerToken", () => {
  it("exchanges an authorization code for an access token and profile", async () => {
  ...
  it("rejects an invalid grant_type before transport", async () => {
  ...
  it("fails with NotFound when client_id does not exist", async () => {
    const error = await runEffect(
      SsoControllerToken({
  ...
  it("fails with UnprocessableEntity when the authorization code is invalid", async () => {
    const error = await runEffect(
      SsoControllerToken({
        client_id: clientId,
        client_secret: clientSecret,
