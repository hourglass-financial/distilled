...
POST https://api.erebor.bank/ach_out
Content-Type: application/json

Create a new Outbound ACH Transfer

Reference: https://docs.erebor.bank/api-reference/payments/ach/outbound/create-outbound-ach-transfer
...
paths:
  /ach_out:
    post:
      operationId: create-outbound-ach-transfer
      summary: Create Outbound ACH Transfer
      description: Create a new Outbound ACH Transfer
      tags:
      ...
      responses:
        '200':
          description: Outbound ACH transfer created successfully
          content:
            application/json:
              schema:
      ...
      requestBody:
        content:
          application/json:
            schema:
              $ref: '#/components/schemas/CreateOutboundACHTransferRequest'
servers:
...
components:
  schemas:
    ...
    CustomFields:
      ...
      title: CustomFields
    CreateOutboundACHTransferRequest:
      type: object
      properties:
        deposit_account_id:
          type: string
          description: Internal deposit account originating the outbound transfer
        counterparty_us_bank_account_id:
          type: string
          description: External US bank account receiving the outbound transfer
        amount:
        ...
        company_entry_description:
          type: string
          description: >-
            Short label included in the ACH batch header and visible to the
            recipient (max 10 chars). Must use NACHA-allowed characters only.
        company_discretionary_data:
          type: string
          description: >-
            Optional field for originator's internal use (max 20 chars). Must
            use NACHA-allowed characters only.
        addenda:
          type:
            - array
            - 'null'
          ...
          description: >-
            Addenda records containing remittance information passed through to
            the recipient (max 80 chars each). Note that only one entry is
            allowed; reach out to support if you'd like to supply multiple.
        service:
          $ref: '#/components/schemas/ACHService'
          description: >-
            Optional: default is we choose fastest service based on the current
            time
        custom_ref:
      ...
      required:
        ...
        - sec_code
        - company_entry_description
      title: CreateOutboundACHTransferRequest
