...
GET https://api.erebor.bank/deposit_accounts

Retrieve a paginated list of Deposit Accounts

Reference: https://docs.erebor.bank/api-reference/accounts/deposit-accounts/list-deposit-accounts
...
paths:
  /deposit_accounts:
    get:
      operationId: list-deposit-accounts
      summary: List Deposit Accounts
      description: Retrieve a paginated list of Deposit Accounts
      tags:
      ...
      parameters:
        ...
        - name: parent_account_id
          in: query
          description: Filter by parent account ID (for virtual DDA accounts)
          required: false
          schema:
            type: string
        - name: custom_ref
      ...
      responses:
        '200':
          description: List of Deposit Accounts
          content:
            application/json:
              schema:
                $ref: >-
                  #/components/schemas/DEPOSIT_ACCOUNTS_listDepositAccounts_Response_200
servers:
...
components:
  schemas:
    ...
    DEPOSIT_ACCOUNTS_listDepositAccounts_Response_200:
      type: object
      properties:
        data:
          type: array
      ...
      required:
        ...
        - page_size
        - url
      title: DEPOSIT_ACCOUNTS_listDepositAccounts_Response_200
  securitySchemes:
    ApiKeyAuth:
      type: apiKey
      in: header
...
```python List of Deposit Accounts
import requests

url = "https://api.erebor.bank/deposit_accounts"
...
```javascript List of Deposit Accounts
const url = 'https://api.erebor.bank/deposit_accounts';
...
```ruby List of Deposit Accounts
require 'uri'
require 'net/http'

url = URI("https://api.erebor.bank/deposit_accounts")
...
```java List of Deposit Accounts
import com.mashape.unirest.http.HttpResponse;
import com.mashape.unirest.http.Unirest;

HttpResponse<String> response = Unirest.get("https://api.erebor.bank/deposit_accounts")
  .header("Authorization", "<apiKey>")
  .asString();
```

```php List of Deposit Accounts
<?php
require_once('vendor/autoload.php');

$client = new \GuzzleHttp\Client();

$response = $client->request('GET', 'https://api.erebor.bank/deposit_accounts', [
  'headers' => [
...
```csharp List of Deposit Accounts
using RestSharp;

var client = new RestClient("https://api.erebor.bank/deposit_accounts");
var request = new RestRequest(Method.GET);
request.AddHeader("Authorization", "<apiKey>");
IRestResponse response = client.Execute(request);
```

```swift List of Deposit Accounts
import Foundation

let headers = ["Authorization": "<apiKey>"]

let request = NSMutableURLRequest(url: NSURL(string: "https://api.erebor.bank/deposit_accounts")! as URL,
