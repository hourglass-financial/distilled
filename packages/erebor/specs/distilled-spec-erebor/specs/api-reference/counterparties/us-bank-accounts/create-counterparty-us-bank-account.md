...
POST https://api.erebor.bank/counterparty_us_bank_accounts
Content-Type: application/json

Create a new US Bank Account for a Counterparty

Reference: https://docs.erebor.bank/api-reference/counterparties/us-bank-accounts/create-counterparty-us-bank-account
...
paths:
  /counterparty_us_bank_accounts:
    post:
      operationId: create-counterparty-us-bank-account
      summary: Create Counterparty US Bank Account
      description: Create a new US Bank Account for a Counterparty
      tags:
      ...
      responses:
        '200':
          description: US Bank Account created successfully
          content:
            application/json:
              schema:
      ...
      requestBody:
        content:
          application/json:
            schema:
              $ref: '#/components/schemas/CreateCounterpartyUSBankAccountRequest'
servers:
...
components:
  schemas:
    ...
    CustomFields:
      ...
      title: CustomFields
    CreateCounterpartyUSBankAccountRequest:
      type: object
      properties:
        counterparty_id:
          type: string
        ...
        routing_number:
          type: string
          description: ACH routing number for US domestic transfers
        custom_ref:
      ...
      required:
        ...
        - account_number
        - routing_number
      title: CreateCounterpartyUSBankAccountRequest
...
### Created Counterparty US Bank Account
...
```python Created Counterparty US Bank Account
import requests
...
```javascript Created Counterparty US Bank Account
...
```go Created Counterparty US Bank Account
package main
...
```ruby Created Counterparty US Bank Account
require 'uri'
require 'net/http'
...
```java Created Counterparty US Bank Account
import com.mashape.unirest.http.HttpResponse;
import com.mashape.unirest.http.Unirest;

HttpResponse<String> response = Unirest.post("https://api.erebor.bank/counterparty_us_bank_accounts")
  .header("Authorization", "<apiKey>")
  .header("Content-Type", "application/json")
  .asString();
```

```php Created Counterparty US Bank Account
<?php
require_once('vendor/autoload.php');

$client = new \GuzzleHttp\Client();

$response = $client->request('POST', 'https://api.erebor.bank/counterparty_us_bank_accounts', [
  'headers' => [
    'Authorization' => '<apiKey>',
    'Content-Type' => 'application/json',
  ],
...
```csharp Created Counterparty US Bank Account
using RestSharp;
...
```swift Created Counterparty US Bank Account
import Foundation
...
```python Create Counterparty US Bank Account
import requests
...
```javascript Create Counterparty US Bank Account
const url = 'https://api.erebor.bank/counterparty_us_bank_accounts';
const options = {
  method: 'POST',
  headers: {Authorization: '<apiKey>', 'Content-Type': 'application/json'},
  body: '{"counterparty_id":"cp_01kasd1tthf1ns1pjn1kncctwd","description":"Primary USD Account","account_number":"123456789","routing_number":"125109161","custom_ref":"CP-USBA-2025-001","custom_fields":{"aba_verified":"true","bank_name":"Wells Fargo"}}'
};
...
```go Create Counterparty US Bank Account
package main
...
```ruby Create Counterparty US Bank Account
require 'uri'
require 'net/http'
...
```java Create Counterparty US Bank Account
import com.mashape.unirest.http.HttpResponse;
import com.mashape.unirest.http.Unirest;
...
```php Create Counterparty US Bank Account
<?php
require_once('vendor/autoload.php');
...
```csharp Create Counterparty US Bank Account
using RestSharp;
...
```swift Create Counterparty US Bank Account
import Foundation
