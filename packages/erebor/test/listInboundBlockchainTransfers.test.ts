/**
 * Tests for the `listInboundBlockchainTransfers` operation.
 *
...
describe("listInboundBlockchainTransfers", () => {
  describe("happy path", () => {
    it(
      "returns a paginated list of inbound blockchain transfers",
      async () => {
        const result = await runEffect(
          listInboundBlockchainTransfers({ page_size: 5 }),
        );
        ...
        for (const transfer of result.data) {
          expect(transfer.type).toBe("BLOCKCHAIN_IN");
    ...
    it(
      ...
      async () => {
        ...
        const error = (await Effect.runPromise(
          listInboundBlockchainTransfers({}).pipe(
            Effect.flip,
            Effect.provide(Main),
