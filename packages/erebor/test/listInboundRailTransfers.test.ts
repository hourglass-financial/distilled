...
describe("listInboundRailTransfers", () => {
  describe("happy path", () => {
    it("returns a paginated list of inbound rail transfers", async () => {
      const result = await runEffect(
        listInboundRailTransfers({ page_size: 5 }),
      );
  ...
  describe("errors", () => {
    it("returns Unauthorized when credentials are invalid", async () => {
      ...
      const error = await Effect.runPromise(
        listInboundRailTransfers({ page_size: 1 }).pipe(
          Effect.flip,
    ...
    it("returns BadRequest for an invalid page_size", async () => {
      const error = await runEffect(
        listInboundRailTransfers({ page_size: -1 }).pipe(Effect.flip),
      );
