/**
 * Tests for the `listDepositAccounts` operation.
 *
...
describe("listDepositAccounts", () => {
  describe("happy path", () => {
    it(
      "returns a paginated list of deposit accounts",
      async () => {
        const result = await runEffect(listDepositAccounts({ page_size: 5 }));
        ...
        for (const account of result.data) {
          expect(account.type).toBe("DEPOSIT_ACCOUNT");
          expect(typeof account.id).toBe("string");
          expect(typeof account.customer_id).toBe("string");
          expect(typeof account.deposit_account_template_id).toBe("string");
          expect(["DDA", "FBO", "OMNIBUS", "VIRTUAL_DDA"]).toContain(
            account.deposit_account_type,
          );
          expect(["BUSINESS", "INDIVIDUAL"]).toContain(account.ownership_type);
          expect(account.balances).toBeTruthy();
        }
      },
      30_000,
    );
    ...
    it(
      ...
      async () => {
        const result = await runEffect(
          listDepositAccounts({ custom_ref: `nonexistent-${testRunId}` }),
        );
    ...
    it(
      ...
      async () => {
        ...
        const error = (await runEffect(
          listDepositAccounts({ starting_after: "not-a-real-cursor" }).pipe(
            Effect.flip,
          ),
        )) as { _tag: string };
