/**
 * Tests for the `listOutboundBlockchainTransfers` operation.
 *
...
describe("listOutboundBlockchainTransfers", () => {
  describe("happy path", () => {
    it(
      "returns a paginated list of outbound blockchain transfers",
      async () => {
        const result = await runEffect(
          listOutboundBlockchainTransfers({ page_size: 5 }),
        );
        ...
        for (const transfer of result.data) {
          expect(transfer.type).toBe("BLOCKCHAIN_OUT");
          expect(typeof transfer.id).toBe("string");
          expect(typeof transfer.deposit_account_id).toBe("string");
          expect(typeof transfer.counterparty_blockchain_address_id).toBe(
            "string",
          );
          expect(["USAT", "USDC", "USDT"]).toContain(transfer.amount.currency);
    ...
    it(
      ...
      async () => {
        const result = await runEffect(
          listOutboundBlockchainTransfers({
    ...
    it(
      ...
      async () => {
        ...
        const error = (await Effect.runPromise(
          listOutboundBlockchainTransfers({}).pipe(
            Effect.flip,
            Effect.provide(Main),
