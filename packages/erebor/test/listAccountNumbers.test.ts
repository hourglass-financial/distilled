/**
 * Tests for the `listAccountNumbers` operation.
 *
...
describe("listAccountNumbers", () => {
  describe("happy path", () => {
    it(
      "returns a paginated list of account numbers",
      async () => {
        const result = await runEffect(listAccountNumbers({ page_size: 5 }));
        ...
        for (const acctNum of result.data) {
          expect(acctNum.type).toBe("ACCOUNT_NUMBER");
          expect(typeof acctNum.id).toBe("string");
          expect(typeof acctNum.deposit_account_id).toBe("string");
          expect(typeof acctNum.account_number).toBe("string");
          expect(typeof acctNum.routing_number).toBe("string");
          expect(typeof acctNum.default).toBe("boolean");
        }
      },
      30_000,
    );
    ...
    it(
      ...
      async () => {
        const result = await runEffect(
          listAccountNumbers({ custom_ref: `nonexistent-${testRunId}` }),
        );
    ...
    it(
      ...
      async () => {
        ...
        const error = (await runEffect(
          listAccountNumbers({ starting_after: "not-a-real-cursor" }).pipe(
            Effect.flip,
          ),
        )) as { _tag: string };
