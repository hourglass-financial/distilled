/**
 * Tests for the `updateCounterpartyInternationalBankAccount` operation.
 *
 * Happy path discovers a real id via
 * `listCounterpartyInternationalBankAccounts` and patches its
 * `description` / `custom_ref` / `custom_fields`. Error coverage hits
...
import { updateCounterpartyInternationalBankAccount } from "../src/operations/updateCounterpartyInternationalBankAccount.ts";
import { runEffect, testRunId, unknownId } from "./setup.ts";

describe("updateCounterpartyInternationalBankAccount", () => {
  describe("happy path", () => {
    it(
      "patches description, custom_ref and custom_fields on an existing counterparty international bank account",
      async () => {
  ...
  describe("errors", () => {
    ...
    it(
      ...
      async () => {
        ...
        // /counterparty_international_bank_accounts/{id}. A clearly
        // malformed path segment exercises that branch.
        const error = (await runEffect(
          updateCounterpartyInternationalBankAccount({
            id: "!!!invalid!!!",
            custom_ref: `distilled-erebor-${testRunId}`,
