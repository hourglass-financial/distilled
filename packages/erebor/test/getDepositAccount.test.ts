/**
 * Tests for the `getDepositAccount` operation.
 *
 * Happy path discovers a real id via `listDepositAccounts`. Error coverage
  ...
  describe("errors", () => {
    it(
      ...
      async () => {
        ...
        const error = (await Effect.runPromise(
          getDepositAccount({ id: unknownId("dpst_acct") }).pipe(
            Effect.flip,
            Effect.provide(Main),
    ...
    it(
      ...
      async () => {
        const error = (await runEffect(
          getDepositAccount({ id: unknownId("dpst_acct") }).pipe(Effect.flip),
        )) as { _tag: string };
    ...
    it(
      ...
      async () => {
        ...
        const error = (await runEffect(
          getDepositAccount({ id: "!!!invalid!!!" }).pipe(Effect.flip),
        )) as { _tag: string };
