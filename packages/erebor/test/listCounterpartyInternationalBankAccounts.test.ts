/**
 * Tests for the `listCounterpartyInternationalBankAccounts` operation.
 *
...
describe("listCounterpartyInternationalBankAccounts", () => {
  describe("happy path", () => {
    it(
      "returns a paginated list of counterparty international bank accounts",
      async () => {
        const result = await runEffect(
          listCounterpartyInternationalBankAccounts({ page_size: 5 }),
        );
        ...
        for (const acct of result.data) {
          expect(acct.type).toBe("COUNTERPARTY_INTERNATIONAL_BANK_ACCOUNT");
          expect(typeof acct.id).toBe("string");
          expect(typeof acct.description).toBe("string");
          expect(typeof acct.account_number).toBe("string");
          expect(typeof acct.bic).toBe("string");
          expect(typeof acct.country_code).toBe("string");
        }
      },
      30_000,
    );
    ...
    it(
      ...
      async () => {
        const result = await runEffect(
          listCounterpartyInternationalBankAccounts({
    ...
    it(
      ...
      async () => {
        ...
        const error = (await Effect.runPromise(
          listCounterpartyInternationalBankAccounts({}).pipe(
            Effect.flip,
            Effect.provide(Main),
