/**
 * Tests for the `listPersonApplicants` operation.
 *
...
describe("listPersonApplicants", () => {
  describe("happy path", () => {
    it(
      "returns a paginated list of person applicants",
      async () => {
        const result = await runEffect(listPersonApplicants({ page_size: 5 }));
        ...
        for (const applicant of result.data) {
          expect(applicant.type).toBe("PERSON_APPLICANT");
          expect(typeof applicant.id).toBe("string");
          expect(typeof applicant.program_id).toBe("string");
          expect(typeof applicant.first_name).toBe("string");
          expect(typeof applicant.last_name).toBe("string");
        }
      },
      30_000,
    );
    ...
    it(
      ...
      async () => {
        const result = await runEffect(
          listPersonApplicants({ custom_ref: `nonexistent-${testRunId}` }),
        );
    ...
    it(
      ...
      async () => {
        ...
        const error = (await runEffect(
          listPersonApplicants({ starting_after: "not-a-real-cursor" }).pipe(
            Effect.flip,
          ),
        )) as { _tag: string };
