...
describe("listInboundWireTransfers", () => {
  describe("happy path", () => {
    it("returns a paginated list of inbound wire transfers", async () => {
      const result = await runEffect(
        listInboundWireTransfers({ page_size: 5 }),
      );
      ...
      for (const transfer of result.data) {
        expect(transfer.type).toBe("WIRE_IN");
  ...
  describe("errors", () => {
    it("returns Unauthorized when credentials are invalid", async () => {
      ...
      const error = await Effect.runPromise(
        listInboundWireTransfers({ page_size: 1 }).pipe(
          Effect.flip,
    ...
    it("returns BadRequest for an invalid page_size", async () => {
      const error = await runEffect(
        listInboundWireTransfers({ page_size: -1 }).pipe(Effect.flip),
      );
