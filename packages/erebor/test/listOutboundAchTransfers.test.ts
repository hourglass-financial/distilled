/**
 * Tests for the `listOutboundAchTransfers` operation.
 *
...
describe("listOutboundAchTransfers", () => {
  describe("happy path", () => {
    it(
      "returns a paginated list of outbound ACH transfers",
      async () => {
        const result = await runEffect(
          listOutboundAchTransfers({ page_size: 5 }),
        );
        ...
        for (const transfer of result.data) {
          expect(transfer.type).toBe("ACH_OUT");
          expect(typeof transfer.id).toBe("string");
          expect(typeof transfer.deposit_account_id).toBe("string");
          expect(typeof transfer.counterparty_us_bank_account_id).toBe(
            "string",
          );
          ...
          expect(["CCD", "PPD", "WEB"]).toContain(transfer.sec_code);
          expect(["SAME_DAY", "STANDARD"]).toContain(transfer.service);
    ...
    it(
      ...
      async () => {
        const result = await runEffect(
          listOutboundAchTransfers({ custom_ref: `nonexistent-${testRunId}` }),
        );
