/**
 * Tests for the `getOnboarding` operation.
 *
 * Happy path discovers a real id via `listOnboardings`. Error coverage
 * hits Forbidden (bad key), NotFound (well-formed-but-missing id), and
 * the BadRequest path declared by the spec patch.
 */
import { Effect, Redacted } from "effect";
import * as Layer from "effect/Layer";
import * as FetchHttpClient from "effect/unstable/http/FetchHttpClient";
import { describe, expect, it } from "vitest";
import { Credentials, DEFAULT_API_BASE_URL } from "../src/credentials.ts";
import { getOnboarding } from "../src/operations/getOnboarding.ts";
import { listOnboardings } from "../src/operations/listOnboardings.ts";
import { runEffect, testRunId, unknownId } from "./setup.ts";

describe("getOnboarding", () => {
  describe("happy path", () => {
    it(
      "fetches an onboarding by id",
      async () => {
        const list = await runEffect(listOnboardings({ page_size: 1 }));
        if (list.data.length === 0) return;

        const target = list.data[0]!;
        const result = await runEffect(getOnboarding({ id: target.id }));

        expect(result.type).toBe("ONBOARDING");
        expect(result.id).toBe(target.id);
        expect(typeof result.program_id).toBe("string");
        expect(["SUBMITTED", "UNDER_REVIEW", "APPROVED", "REJECTED"]).toContain(
          result.status,
        );
        expect(["PERSON", "BUSINESS"]).toContain(result.applicant_type);
        // `disclosures` is now optional per the spec patch and is absent from
        // live responses, so we no longer assert on its presence.
      },
      30_000,
    );
  });

  describe("errors", () => {
    it(
      "invalid API key -> Forbidden",
      async () => {
        const BadCreds = Layer.succeed(Credentials, {
          apiKey: Redacted.make(`test_key_invalid_${testRunId}`),
          apiBaseUrl: DEFAULT_API_BASE_URL,
        });
        const Main = Layer.merge(BadCreds, FetchHttpClient.layer);

        const error = (await Effect.runPromise(
          getOnboarding({ id: unknownId("onbrdng") }).pipe(
            Effect.flip,
            Effect.provide(Main),
          ) as Effect.Effect<unknown, never, never>,
        )) as { _tag: string };

        expect(error._tag).toBe("Forbidden");
      },
      30_000,
    );

    it(
      "unknown onboarding id -> NotFound",
      async () => {
        const error = (await runEffect(
          getOnboarding({ id: unknownId("onbrdng") }).pipe(Effect.flip),
        )) as { _tag: string };

        expect(error._tag).toBe("NotFound");
      },
      30_000,
    );

    it(
      "malformed onboarding id -> BadRequest",
      async () => {
        // Spec patch declares 400 INVALID_REQUEST on GET /onboardings/{id}.
        // A clearly malformed path segment exercises that branch.
        const error = (await runEffect(
          getOnboarding({ id: "!!!invalid!!!" }).pipe(Effect.flip),
        )) as { _tag: string };

        expect(error._tag).toBe("BadRequest");
      },
      30_000,
    );
  });
});
