/**
 * Tests for the `updateDepositAccount` operation.
 *
 * Happy path discovers a real id via `listDepositAccounts` and patches its
 * `name` / `custom_ref` / `custom_fields`. Error coverage hits Forbidden
 * (bad key), NotFound (well-formed-but-missing id), and BadRequest
 * (malformed id).
 */
...
import { updateDepositAccount } from "../src/operations/updateDepositAccount.ts";
import { runEffect, testRunId, unknownId } from "./setup.ts";

describe("updateDepositAccount", () => {
  describe("happy path", () => {
    it(
      "patches name, custom_ref and custom_fields on an existing deposit account",
      async () => {
        const list = await runEffect(listDepositAccounts({ page_size: 1 }));
        if (list.data.length === 0) return;

        const target = list.data[0]!;
        const newName = `distilled-erebor-da-${testRunId}`;
  ...
  describe("errors", () => {
    ...
    it(
      "unknown deposit account id -> NotFound",
      async () => {
    ...
    it(
      ...
      async () => {
        ...
        // /deposit_accounts/{id}. A clearly malformed path segment
