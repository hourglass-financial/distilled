/**
 * Tests for the `updateAccountNumber` operation.
 *
 * Happy path discovers a real id via `listAccountNumbers` and patches its
...
import { updateAccountNumber } from "../src/operations/updateAccountNumber.ts";
import { runEffect, testRunId, unknownId } from "./setup.ts";

describe("updateAccountNumber", () => {
  describe("happy path", () => {
    it(
      "patches name, custom_ref and custom_fields on an existing account number",
      async () => {
        const list = await runEffect(listAccountNumbers({ page_size: 1 }));
        if (list.data.length === 0) return;

        const target = list.data[0]!;
        const newName = `distilled-erebor-acctnum-${testRunId}`;
