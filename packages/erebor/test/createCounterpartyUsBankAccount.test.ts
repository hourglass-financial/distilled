...
// Account numbers must be digits-only and at most 17 chars. The same
// account_number + routing_number can only be attached to one party, so a
// fixed value collides (409 Conflict) on re-run. Derive a unique, digits-only
// account_number from the per-run id by mapping each hex char to a digit.
const uniqueAccountNumber = (): string => {
  const digits = testRunId
    .split("")
    .map((c) => (parseInt(c, 16) % 10).toString())
    .join("");
  // 8 hex chars -> 8 digits; pad to a realistic length, stay under 17.
  return `0001${digits}`;
};
...
describe("createCounterpartyUsBankAccount", () => {
  describe("happy path", () => {
    it("creates a US bank account for an existing counterparty", async () => {
      ...
      expect(result.description).toBe(`Distilled test US bank ${testRunId}`);
  ...
  describe("errors", () => {
    it("invalid API key -> Forbidden", async () => {
      ...
      const error = (await Effect.runPromise(
        createCounterpartyUsBankAccount({
          ...
          routing_number: VALID_ROUTING_NUMBER,
        }).pipe(Effect.flip, Effect.provide(Main)) as Effect.Effect<
          unknown,
          never,
          never
        >,
    ...
    it("unknown counterparty id -> BadRequest", async () => {
    ...
    it("malformed counterparty id -> BadRequest", async () => {
    ...
    it("routing number that fails ABA checksum -> BadRequest", async () => {
