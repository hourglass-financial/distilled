/**
 * Tests for the `getCounterpartyBlockchainAddress` operation.
 *
...
import { getCounterpartyBlockchainAddress } from "../src/operations/getCounterpartyBlockchainAddress.ts";
import { listCounterpartyBlockchainAddresses } from "../src/operations/listCounterpartyBlockchainAddresses.ts";
import { runEffect, testRunId, unknownId } from "./setup.ts";

describe("getCounterpartyBlockchainAddress", () => {
  describe("happy path", () => {
    it(
      "fetches a counterparty blockchain address by id",
      async () => {
        ...
        const result = await runEffect(
          getCounterpartyBlockchainAddress({ id: target.id }),
        );
        ...
        expect(typeof result.custodian).toBe("string");
      },
      30_000,
    );
  });
  ...
  describe("errors", () => {
    it(
      ...
      async () => {
        ...
        const error = (await Effect.runPromise(
          getCounterpartyBlockchainAddress({ id: unknownId("cpba") }).pipe(
            Effect.flip,
            Effect.provide(Main),
    ...
    it(
      ...
      async () => {
        ...
        const error = (await runEffect(
          getCounterpartyBlockchainAddress({ id: "!!!invalid!!!" }).pipe(
            Effect.flip,
          ),
        )) as { _tag: string };
