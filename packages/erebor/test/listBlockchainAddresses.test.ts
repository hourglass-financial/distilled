/**
 * Tests for the `listBlockchainAddresses` operation.
 *
...
describe("listBlockchainAddresses", () => {
  describe("happy path", () => {
    it(
      "returns a paginated list of blockchain addresses",
      async () => {
        const result = await runEffect(
          listBlockchainAddresses({ page_size: 5 }),
        );
        ...
        for (const addr of result.data) {
          expect(addr.type).toBe("BLOCKCHAIN_ADDRESS");
          expect(typeof addr.id).toBe("string");
          expect(typeof addr.deposit_account_id).toBe("string");
          expect(typeof addr.address).toBe("string");
          expect(["ETHEREUM", "SOLANA", "SUI"]).toContain(addr.address_type);
          expect(Array.isArray(addr.network)).toBe(true);
        }
      },
      30_000,
    );
    ...
    it(
      ...
      async () => {
        const result = await runEffect(
          listBlockchainAddresses({ custom_ref: `nonexistent-${testRunId}` }),
        );
    ...
    it(
      ...
      async () => {
        ...
        const error = (await runEffect(
          listBlockchainAddresses({ starting_after: "not-a-real-cursor" }).pipe(
            Effect.flip,
          ),
        )) as { _tag: string };
