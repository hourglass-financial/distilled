/**
 * Tests for the `listOnboardings` operation.
 *
 * Exercises paginated retrieval against the Erebor sandbox plus the
 * auth-failure (Forbidden) and malformed-cursor (BadRequest) paths
 * surfaced by the client's matchError mapping.
 */
import { Effect, Redacted } from "effect";
import * as Layer from "effect/Layer";
import * as FetchHttpClient from "effect/unstable/http/FetchHttpClient";
import { describe, expect, it } from "vitest";
import { Credentials, DEFAULT_API_BASE_URL } from "../src/credentials.ts";
import { listOnboardings } from "../src/operations/listOnboardings.ts";
import { runEffect, testRunId } from "./setup.ts";

describe("listOnboardings", () => {
  describe("happy path", () => {
    it(
      "returns a paginated list of onboardings",
      async () => {
        const result = await runEffect(listOnboardings({ page_size: 5 }));

        expect(result).toBeTruthy();
        expect(Array.isArray(result.data)).toBe(true);
        expect(typeof result.has_more).toBe("boolean");
        expect(typeof result.page_size).toBe("number");
        expect(result.page_size).toBeLessThanOrEqual(5);
        for (const onboarding of result.data) {
          expect(onboarding.type).toBe("ONBOARDING");
          expect(typeof onboarding.id).toBe("string");
          expect(typeof onboarding.program_id).toBe("string");
          expect(["SUBMITTED", "UNDER_REVIEW", "APPROVED", "REJECTED"]).toContain(
            onboarding.status,
          );
          expect(["PERSON", "BUSINESS"]).toContain(onboarding.applicant_type);
        }
      },
      30_000,
    );

    it(
      "custom_ref filter that matches nothing returns an empty page",
      async () => {
        const result = await runEffect(
          listOnboardings({ custom_ref: `nonexistent-${testRunId}` }),
        );

        expect(result.data).toEqual([]);
        expect(result.has_more).toBe(false);
      },
      30_000,
    );
  });

  describe("errors", () => {
    it(
      "invalid API key -> Forbidden",
      async () => {
        const BadCreds = Layer.succeed(Credentials, {
          apiKey: Redacted.make(`test_key_invalid_${testRunId}`),
          apiBaseUrl: DEFAULT_API_BASE_URL,
        });
        const Main = Layer.merge(BadCreds, FetchHttpClient.layer);

        const error = (await Effect.runPromise(
          listOnboardings({}).pipe(
            Effect.flip,
            Effect.provide(Main),
          ) as Effect.Effect<unknown, never, never>,
        )) as { _tag: string };

        expect(error._tag).toBe("Forbidden");
      },
      30_000,
    );

  });
});
