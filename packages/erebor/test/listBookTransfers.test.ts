/**
 * Tests for the `listBookTransfers` operation.
 *
...
describe("listBookTransfers", () => {
  describe("happy path", () => {
    it(
      "returns a paginated list of book transfers",
      async () => {
        const result = await runEffect(listBookTransfers({ page_size: 5 }));
    ...
    it(
      ...
      async () => {
        const result = await runEffect(
          listBookTransfers({ custom_ref: `nonexistent-${testRunId}` }),
        );
