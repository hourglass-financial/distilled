/**
 * Tests for the `getBookTransfer` operation.
 *
 * Happy path discovers a real id via `listBookTransfers`. Error coverage
...
import { getBookTransfer } from "../src/operations/getBookTransfer.ts";
import { listBookTransfers } from "../src/operations/listBookTransfers.ts";
import { runEffect, testRunId, unknownId } from "./setup.ts";

describe("getBookTransfer", () => {
  describe("happy path", () => {
    it(
      "fetches a book transfer by id",
      async () => {
        const list = await runEffect(listBookTransfers({ page_size: 1 }));
        if (list.data.length === 0) return;

        const target = list.data[0]!;
        const result = await runEffect(getBookTransfer({ id: target.id }));
  ...
  describe("errors", () => {
    it(
      ...
      async () => {
        ...
        const error = (await Effect.runPromise(
          getBookTransfer({ id: unknownId("book_xfer") }).pipe(
            Effect.flip,
            Effect.provide(Main),
    ...
    it(
      ...
      async () => {
        const error = (await runEffect(
          getBookTransfer({ id: unknownId("book_xfer") }).pipe(Effect.flip),
        )) as { _tag: string };
    ...
    it(
      ...
      async () => {
        ...
        // that branch.
        const error = (await runEffect(
          getBookTransfer({ id: "!!!invalid!!!" }).pipe(Effect.flip),
        )) as { _tag: string };
