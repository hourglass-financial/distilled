/**
 * Tests for the `listCounterpartyBlockchainAddresses` operation.
 *
...
describe("listCounterpartyBlockchainAddresses", () => {
  describe("happy path", () => {
    it(
      "returns a paginated list of counterparty blockchain addresses",
      async () => {
        const result = await runEffect(
          listCounterpartyBlockchainAddresses({ page_size: 5 }),
        );
        ...
        for (const addr of result.data) {
          expect(addr.type).toBe("COUNTERPARTY_BLOCKCHAIN_ADDRESS");
          expect(typeof addr.id).toBe("string");
          expect(typeof addr.description).toBe("string");
          expect(typeof addr.address).toBe("string");
          expect(typeof addr.network).toBe("string");
          expect(typeof addr.custodian).toBe("string");
        }
      },
      30_000,
    );
    ...
    it(
      ...
      async () => {
        const result = await runEffect(
          listCounterpartyBlockchainAddresses({
    ...
    it(
      ...
      async () => {
        ...
        const error = (await Effect.runPromise(
          listCounterpartyBlockchainAddresses({}).pipe(
            Effect.flip,
            Effect.provide(Main),
