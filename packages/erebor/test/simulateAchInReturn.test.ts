...
import { simulateAchInReturn } from "../src/operations/simulateAchInReturn.ts";
...
const findSimulatable = async (
  ...
  const list = await runEffect(listInboundAchTransfers({ page_size: 100 }));
  const candidates = list.data.filter((t) =>
    status === "SETTLED" ? t.status === "SETTLED" : t.status !== "SETTLED",
  );
  for (const t of candidates) {
    const result = await Effect.runPromise(
      simulateAchInReturn({ id: t.id, return_code: returnCode }).pipe(
        Effect.provide(
          Layer.merge(
...
describe("simulateAchInReturn", () => {
  describe("happy path", () => {
    it("simulates a return on a settled inbound ACH transfer", async () => {
      const list = await runEffect(
        listInboundAchTransfers({ page_size: 100, status: "SETTLED" }),
      );
      for (const t of list.data.filter((x) => x.status === "SETTLED")) {
        const out = await Effect.runPromise(
          simulateAchInReturn({ id: t.id, return_code: "R01" }).pipe(
            Effect.match({
              onSuccess: (r) => r,
              onFailure: (e: { _tag: string }) =>
  ...
  describe("errors", () => {
    ...
    it("returns BadRequest for an invalid return_code on a reachable transfer", async (ctx) => {
      ...
      if (reachable.tag === "Conflict") {
        ctx.skip(
          "No SETTLED transfer in sandbox scope reaches return_code validation.",
        );
        return;
      }
      ...
      const outcome = await runEffect(
        simulateAchInReturn({ id: reachable.id, return_code: "R99" }).pipe(
          Effect.match({
