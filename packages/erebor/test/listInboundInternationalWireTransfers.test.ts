...
describe("listInboundInternationalWireTransfers", () => {
  describe("happy path", () => {
    it("returns a paginated list of inbound international wire transfers", async () => {
      const result = await runEffect(
        listInboundInternationalWireTransfers({ page_size: 5 }),
      );
      ...
      for (const transfer of result.data) {
        expect(transfer.type).toBe("INTERNATIONAL_WIRE_IN");
  ...
  describe("errors", () => {
    it("returns Unauthorized when credentials are invalid", async () => {
      ...
      const error = await Effect.runPromise(
        listInboundInternationalWireTransfers({ page_size: 1 }).pipe(
          Effect.flip,
