...
To generate an SDK, clone the distilled repo, run `bun install`, then run the
`create-sdk-full` command.

Every SDK has all of its sources as git submodules; you can either supply a git
repo or an HTTP URL to an OpenAPI spec. HTTP specs are mirrored into a repo that
updates nightly. Use `--note` to specify where the API spec lives in the repo or
to restrict scope.

```bash
bun scripts/create-sdk-full.ts discord --specs https://github.com/discord/discord-api-spec --note "use the spec in /specs/openapi.json from the provided repo"
```

The pipeline is coordinated by repo-local Smithers workflows. The workflows run
deterministic scaffold/check steps and scoped Codex tasks for generator
refinement, error discovery, tests, and nuke-script generation.

For larger SDKs, you can rerun the command and use skip flags for completed
stages. See `bun scripts/create-sdk-full.ts --help`.
