...
describe("matcher tables", () => {
  ...
  it("the fallback and wrapper errors are classified", () => {
    expect(
      Category.metaOf(
        new UnknownGlyphError({ message: "x", body: Redacted.make(null) }),
      ),
    ).toEqual({ category: "unknown", retry: "none" });
    expect(
      Category.metaOf(new GlyphTransportError({ message: "x", cause: null })),
    ).toEqual({ category: "transport", retry: "transient" });
    expect(
      Category.metaOf(
        new GlyphDecodeError({ message: "x", cause: Redacted.make(null) }),
      ),
