 ...
 *   - error codes / status table → the Glyph OpenAPI spec + generator mapping
...
/**
 * A Glyph error the SDK does not (yet) model — an unmapped status or an
 ...
 */
export class UnknownGlyphError extends Schema.TaggedErrorClass<UnknownGlyphError>()(
  "UnknownGlyphError",
  {
 ...
 */
export class GlyphTransportError extends Schema.TaggedErrorClass<GlyphTransportError>()(
  "GlyphTransportError",
 ...
 */
export class GlyphDecodeError extends Schema.TaggedErrorClass<GlyphDecodeError>()(
  "GlyphDecodeError",
  {
    message: Schema.String,
...
/**
 * Glyph discriminator code → typed error class, derived from each class's
...
/** Errors that can arise on any Glyph operation (401/429/5xx). */
export const DEFAULT_ERRORS = CORE_DEFAULT_ERRORS;

/** Instance union of {@link DEFAULT_ERRORS}. */
export type GlyphUniversalError = InstanceType<(typeof DEFAULT_ERRORS)[number]>;

/**
 * Everything a Glyph operation can fail with *beyond* its own declared typed
