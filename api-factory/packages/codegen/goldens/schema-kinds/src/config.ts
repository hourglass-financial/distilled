...
export const config: Config.Config<GlyphConfig> = Config.all({
  apiKey: Config.redacted("GLYPH_API_KEY"),
  baseUrl: Config.string("GLYPH_API_URL").pipe(
  ...
  Layer.effect(
    ...
    config.pipe(
      Effect.mapError(
        () =>
          new ConfigError({
            message: "Glyph credentials are not configured.",
          }),
      ),
