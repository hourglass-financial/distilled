 ...
 *
 * Every operation flows through the single `RecordJsonClient.run`, assembled here
...
import {
  CODE_ERRORS,
  DEFAULT_ERRORS,
  RecordJsonDecodeError,
  type RecordJsonExtraError,
  RecordJsonTransportError,
  STATUS_ERRORS,
  UnknownRecordJsonError,
} from "./errors.ts";
...
const adapters = makeVendorAdapters<RecordJsonExtraError>({
  UnknownError: UnknownRecordJsonError,
  TransportError: RecordJsonTransportError,
  DecodeError: RecordJsonDecodeError,
});

const matchError = makeMatchError<RecordJsonExtraError>({
  /**
   * Normalize Record JSON's two error envelopes into one shape. `code` and the
...
/**
 * The full, honest error channel of a Record JSON operation: its own declared typed
 ...
 */
export type RecordJsonError<EC extends readonly ClassifiedErrorClass[]> =
  | InstanceType<EC[number]>
  | RecordJsonExtraError;

/** The single request runner every Record JSON operation is dispatched through. */
export interface RecordJsonClientShape {
  readonly run: Runner<RecordJsonExtraError>;
}

/** Context service exposing the Record JSON request runner. */
export class RecordJsonClient extends Context.Service<
  RecordJsonClient,
  RecordJsonClientShape
>()("@hourglass-financial/api-factory-record-json/RecordJsonClient") {}

/** Options for building the client layer. */
export interface RecordJsonClientOptions {
...
export const layerWith = (
  options: RecordJsonClientOptions = {},
): Layer.Layer<RecordJsonClient, never, HttpClient | Credentials> =>
  Layer.effect(
    RecordJsonClient,
    Effect.gen(function* () {
      const http = yield* HttpClient;
      const { apiKey, baseUrl } = yield* Credentials;
      const run = makeRunner<RecordJsonExtraError>({
        http,
        apiKey,
        baseUrl,
      ...
      return RecordJsonClient.of({ run });
    }),
  );
...
>(
  ...
  input: IS["Type"],
): Effect.Effect<OS["Type"], RecordJsonError<EC>, RecordJsonClient> =>
  Effect.flatMap(RecordJsonClient, (client) => client.run(op, input));
