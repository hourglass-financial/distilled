 ...
 *   - error codes / status table → the Record JSON OpenAPI spec + generator mapping
...
/**
 * A Record JSON error the SDK does not (yet) model — an unmapped status or an
 ...
 */
export class UnknownRecordJsonError extends Schema.TaggedErrorClass<UnknownRecordJsonError>()(
 ...
 */
export class RecordJsonTransportError extends Schema.TaggedErrorClass<RecordJsonTransportError>()(
 ...
 */
export class RecordJsonDecodeError extends Schema.TaggedErrorClass<RecordJsonDecodeError>()(
...
/**
 * Record JSON discriminator code → typed error class, derived from each class's
...
/** Errors that can arise on any Record JSON operation (401/429/5xx). */
export const DEFAULT_ERRORS = CORE_DEFAULT_ERRORS;

/** Instance union of {@link DEFAULT_ERRORS}. */
export type RecordJsonUniversalError = InstanceType<
  (typeof DEFAULT_ERRORS)[number]
>;

/**
 * Everything a Record JSON operation can fail with *beyond* its own declared typed
 ...
 */
export type RecordJsonExtraError =
  | RecordJsonUniversalError
  | UnknownRecordJsonError
  | RecordJsonTransportError
  | RecordJsonDecodeError;
