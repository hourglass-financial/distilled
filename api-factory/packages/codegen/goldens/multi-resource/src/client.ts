 ...
 *
 * Every operation flows through the single `MosaicClient.run`, assembled here
...
const adapters = makeVendorAdapters<MosaicExtraError>({
  UnknownError: UnknownMosaicError,
  TransportError: MosaicTransportError,
  DecodeError: MosaicDecodeError,
});

const matchError = makeMatchError<MosaicExtraError>({
  /**
   * Normalize Mosaic's two error envelopes into one shape. `code` and the
...
/**
 * The full, honest error channel of a Mosaic operation: its own declared typed
 ...
 */
export type MosaicError<EC extends readonly ClassifiedErrorClass[]> =
  | InstanceType<EC[number]>
  | MosaicExtraError;

/** The single request runner every Mosaic operation is dispatched through. */
export interface MosaicClientShape {
  readonly run: Runner<MosaicExtraError>;
}

/** Context service exposing the Mosaic request runner. */
export class MosaicClient extends Context.Service<
  MosaicClient,
  MosaicClientShape
>()("@hourglass-financial/api-factory-mosaic/MosaicClient") {}
...
): Layer.Layer<MosaicClient, never, HttpClient | Credentials> =>
  Layer.effect(
    MosaicClient,
    Effect.gen(function* () {
      const http = yield* HttpClient;
      const { apiKey, baseUrl } = yield* Credentials;
      const run = makeRunner<MosaicExtraError>({
        http,
        apiKey,
        baseUrl,
      ...
      return MosaicClient.of({ run });
    }),
  );

/** Client layer with the default retry policy, over injected deps. */
export const layer: Layer.Layer<MosaicClient, never, HttpClient | Credentials> =
  layerWith();
...
>(
  ...
  input: IS["Type"],
): Effect.Effect<OS["Type"], MosaicError<EC>, MosaicClient> =>
  Effect.flatMap(MosaicClient, (client) => client.run(op, input));
