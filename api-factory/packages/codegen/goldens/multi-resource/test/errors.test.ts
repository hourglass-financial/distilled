 ...
 *   - error codes / tables → the Mosaic OpenAPI spec + generator mapping
 *
 ...
 * accept its own wire code. Behavioral coverage lives in `vendors/mosaic`.
 */
import {
  Category,
...
describe("matcher tables", () => {
  ...
  it("the fallback and wrapper errors are classified", () => {
    expect(
      Category.metaOf(
        new UnknownMosaicError({ message: "x", body: Redacted.make(null) }),
      ),
    ).toEqual({ category: "unknown", retry: "none" });
    expect(
      Category.metaOf(new MosaicTransportError({ message: "x", cause: null })),
    ).toEqual({ category: "transport", retry: "transient" });
    expect(
      Category.metaOf(
        new MosaicDecodeError({ message: "x", cause: Redacted.make(null) }),
      ),
