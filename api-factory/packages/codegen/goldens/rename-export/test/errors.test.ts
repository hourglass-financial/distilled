 ...
 *   - error codes / tables → the Quarry OpenAPI spec + generator mapping
 *
 ...
 * accept its own wire code. Behavioral coverage lives in `vendors/quarry`.
 */
import {
  Category,
...
describe("matcher tables", () => {
  ...
  it("the fallback and wrapper errors are classified", () => {
    expect(
      Category.metaOf(
        new UnknownQuarryError({ message: "x", body: Redacted.make(null) }),
      ),
    ).toEqual({ category: "unknown", retry: "none" });
    expect(
      Category.metaOf(new QuarryTransportError({ message: "x", cause: null })),
    ).toEqual({ category: "transport", retry: "transient" });
    expect(
      Category.metaOf(
        new QuarryDecodeError({ message: "x", cause: Redacted.make(null) }),
      ),
