 ...
 *
 * `@hourglass-financial/api-factory-quarry` — the Quarry client.
 *
 * Provide one layer and call tree-shakeable operations grouped by resource:
 *
 * ```ts
 * import { artifacts, layerFromEnv } from "@hourglass-financial/api-factory-quarry";
 * import { Effect } from "effect";
 *
 * const program = artifacts.delete({});
 * program.pipe(Effect.provide(layerFromEnv));
 * ```
 */

// Resource operation namespaces.
export * as artifacts from "./resources/artifacts.ts";
...
export {
  ...
  layerWith,
  run,
  type QuarryClientOptions,
  type QuarryClientShape,
  QuarryClient,
  type QuarryError,
} from "./client.ts";
