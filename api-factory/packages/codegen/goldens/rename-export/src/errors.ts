 ...
 *   - error codes / status table → the Quarry OpenAPI spec + generator mapping
...
/**
 * A Quarry error the SDK does not (yet) model — an unmapped status or an
 ...
 */
export class UnknownQuarryError extends Schema.TaggedErrorClass<UnknownQuarryError>()(
  "UnknownQuarryError",
  {
 ...
 */
export class QuarryTransportError extends Schema.TaggedErrorClass<QuarryTransportError>()(
 ...
 */
export class QuarryDecodeError extends Schema.TaggedErrorClass<QuarryDecodeError>()(
  "QuarryDecodeError",
  {
    message: Schema.String,
...
/**
 * Quarry discriminator code → typed error class, derived from each class's
...
/** Errors that can arise on any Quarry operation (401/429/5xx). */
export const DEFAULT_ERRORS = CORE_DEFAULT_ERRORS;

/** Instance union of {@link DEFAULT_ERRORS}. */
export type QuarryUniversalError = InstanceType<
  (typeof DEFAULT_ERRORS)[number]
>;

/**
 * Everything a Quarry operation can fail with *beyond* its own declared typed
 ...
 */
export type QuarryExtraError =
  | QuarryUniversalError
  | UnknownQuarryError
  | QuarryTransportError
  | QuarryDecodeError;
