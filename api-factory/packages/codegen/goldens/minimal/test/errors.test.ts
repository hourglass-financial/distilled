...
describe("matcher tables", () => {
  ...
  it("the fallback and wrapper errors are classified", () => {
    expect(
      Category.metaOf(
        new UnknownNorthstarError({ message: "x", body: Redacted.make(null) }),
      ),
    ).toEqual({ category: "unknown", retry: "none" });
    expect(
      Category.metaOf(
        new NorthstarTransportError({ message: "x", cause: null }),
      ),
    ).toEqual({ category: "transport", retry: "transient" });
    expect(
      Category.metaOf(
        new NorthstarDecodeError({ message: "x", cause: Redacted.make(null) }),
      ),
