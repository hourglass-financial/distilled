 ...
 *   - error codes / status table → the Northstar OpenAPI spec + generator mapping
...
/**
 * A Northstar error the SDK does not (yet) model — an unmapped status or an
 ...
 */
export class UnknownNorthstarError extends Schema.TaggedErrorClass<UnknownNorthstarError>()(
 ...
 */
export class NorthstarTransportError extends Schema.TaggedErrorClass<NorthstarTransportError>()(
 ...
 */
export class NorthstarDecodeError extends Schema.TaggedErrorClass<NorthstarDecodeError>()(
...
/**
 * Northstar discriminator code → typed error class, derived from each class's
...
/** Errors that can arise on any Northstar operation (401/429/5xx). */
export const DEFAULT_ERRORS = CORE_DEFAULT_ERRORS;

/** Instance union of {@link DEFAULT_ERRORS}. */
export type NorthstarUniversalError = InstanceType<
  (typeof DEFAULT_ERRORS)[number]
>;

/**
 * Everything a Northstar operation can fail with *beyond* its own declared typed
 ...
 */
export type NorthstarExtraError =
  | NorthstarUniversalError
  | UnknownNorthstarError
  | NorthstarTransportError
  | NorthstarDecodeError;
