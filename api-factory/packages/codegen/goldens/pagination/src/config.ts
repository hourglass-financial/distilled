...
export const config: Config.Config<OrbitConfig> = Config.all({
  apiKey: Config.redacted("ORBIT_API_KEY"),
  baseUrl: Config.string("ORBIT_API_URL").pipe(
  ...
  Layer.effect(
    ...
    config.pipe(
      Effect.mapError(
        () =>
          new ConfigError({
            message: "Orbit credentials are not configured.",
          }),
      ),
