...
PATCH https://api.erebor.bank/webhooks/{id}
...
Reference: https://docs.erebor.bank/api-reference/webhooks/update-webhook
...
paths:
  /webhooks/{id}:
    patch:
      ...
      description: >-
        Update a webhook's `name`, `webhook_url`, `event_types`, or `status`.
        Toggle `status` between `ENABLED` and `DISABLED` to pause and resume
        delivery. To archive a webhook, use `POST /webhooks/{id}/archive`.
      tags:
        - subpackage_webhooks
      parameters:
        - name: id
  ...
  schemas:
    UpdateWebhookRequestStatus:
      ...
      enum:
        ...
        - DISABLED
      description: >-
        Set to `ENABLED` to resume delivery or `DISABLED` to pause. To archive a
        webhook, call `POST /webhooks/{id}/archive`.
      title: UpdateWebhookRequestStatus
    WebhookEventType:
      type: string
      enum:
    ...
    UpdateWebhookRequest:
      ...
      properties:
        ...
        webhook_url:
          ...
          format: uri
        status:
          $ref: '#/components/schemas/UpdateWebhookRequestStatus'
          description: >-
            Set to `ENABLED` to resume delivery or `DISABLED` to pause. To
            archive a webhook, call `POST /webhooks/{id}/archive`.
        event_types:
          type:
            - array
            - 'null'
          ...
          description: >-
            Pass ["*"] to subscribe to all event types, or `null` to clear the
            subscription. See [Supported
...
url = "https://api.erebor.bank/webhooks/id"
...
```javascript
const url = 'https://api.erebor.bank/webhooks/id';
const options = {
  method: 'PATCH',
  headers: {Authorization: '<apiKey>', 'Content-Type': 'application/json'},
  body: '{"status":"DISABLED","custom_ref":"WHK-PROD-NOTIFICATIONS","custom_fields":{"environment":"production","team":"platform"}}'
};
...
url = URI("https://api.erebor.bank/webhooks/id")
...
HttpResponse<String> response = Unirest.patch("https://api.erebor.bank/webhooks/id")
...
$response = $client->request('PATCH', 'https://api.erebor.bank/webhooks/id', [
  'body' => '{
  "status": "DISABLED",
  "custom_ref": "WHK-PROD-NOTIFICATIONS",
  "custom_fields": {
...
var client = new RestClient("https://api.erebor.bank/webhooks/id");
...
let request = NSMutableURLRequest(url: NSURL(string: "https://api.erebor.bank/webhooks/id")! as URL,
